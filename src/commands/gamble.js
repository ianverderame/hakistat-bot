  ...
  async execute(message, args) {
    ...
    if (!args[0] || isNaN(bet) || bet <= 0) {
      return message.channel.send(
        "Please provide a valid amount to gamble. Usage: `!gamble <points>`",
      );
    }
    ...
    try {
      ...
      if (!isNegative) {
        ...
        const todayProfit = todayLogs.reduce(
          (sum, log) => sum + log.points_delta,
          0,
        );
      ...
      const winChance = isNegative
        ? NEGATIVE_BALANCE_WIN_CHANCE
        : DEFAULT_WIN_CHANCE;
      ...
      if (logError) {
        ...
        return message.channel.send(
          "Points were updated but failed to log transaction.",
        );
      }
    } catch (err) {
      console.error(err);
